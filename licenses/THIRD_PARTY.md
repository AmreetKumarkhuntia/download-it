...
## Before distributing binary installers

The repository and CI validate source and development builds. Binary publication is a separate release step: collect the licenses and corresponding source (including distribution patches and build scripts) for the exact aria2 binary and every bundled library, and publish them alongside the installers. A link to upstream master is not a substitute for corresponding source. Linux system aria2 builds and macOS Homebrew builds can include additional LGPL/GPL libraries. Do not publish those libraries with only this summary notice.

Run `pnpm prepare:aria2` on each target platform, audit its manifest and dependency list, retain the binary/source hashes, and follow docs/architecture/releases.md. No release workflow automatically publishes installers.
