...
import { DownloadDetailsWidget } from '../../../apps/desktop/src/widgets/downloads/DownloadDetailsWidget';

const client = vi.hoisted(() => ({ details: vi.fn(), diagnostics: vi.fn(), copyText: vi.fn() }));
vi.mock('../../../apps/desktop/src/services/client/download-client', () => ({
  client,
  errorMessage: (e: Error) => e.message,
}));
vi.mock('../../../apps/desktop/src/services/client/platform-client', () => ({
  desktopAvailable: true,
  platform: { copyText: client.copyText },
}));
...
it('shows actual connection limits, piece completion, source metadata and scoped logs', async () => {
  render(<DownloadDetailsWidget id="download-id" onClose={vi.fn()} />);
...
it('stops details and log refreshes when the dialog closes', async () => {
  vi.useFakeTimers();
  try {
    const { unmount } = render(<DownloadDetailsWidget id="download-id" onClose={vi.fn()} />);
    await act(async () => {});
    const log = screen.getByText('Download log').closest('details')!;
    await act(async () => {
      log.open = true;
      fireEvent(log, new Event('toggle'));
    });
    expect(client.diagnostics).toHaveBeenCalledTimes(1);
